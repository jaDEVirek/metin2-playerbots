...
# Set once per login by the server's own "SetGMFlag" CHAT_TYPE_COMMAND -
# the client has no other way to know its own GM level, so this is not
# trusted for anything except showing the F9 panel; every action the panel
# sends is re-checked against the real gm_level server-side.
IsGM = False
